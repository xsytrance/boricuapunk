...
export default function FigurineReviewPage() {
  ...
  const options = useMemo(
    () => [
      { id: "unknown", name: "Unknown Figurine" },
    ...
    async function load() {
      try {
        const response = await fetch(
          `/api/archive/figurines?needsReview=1&threshold=${REVIEW_THRESHOLD}&limit=80`,
          { cache: "no-store" },
        );
  ...
  async function assign(row: FigurineSighting) {
    const characterId = selectedById[row.id] ?? row.card.id;
    setSavingId(row.id);
    setStatus("");
    try {
      const response = await fetch("/api/archive/figurines", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ sightingId: row.id, characterId }),
      });
      if (!response.ok) {
        const payload = (await response.json().catch(() => ({}))) as { error?: string };
        setStatus(payload.error || "Failed to save figurine assignment.");
        return;
      }

      setRows((prev) => prev.filter((entry) => entry.id !== row.id));
      setStatus("Figurine assignment saved.");
    } catch {
      setStatus("Network error while saving figurine assignment.");
    } finally {
      setSavingId(null);
    }
  }
  ...
  return (
    <ArchivePageShell
      eyebrow="Figurine Review"
      title="Needs Review"
      description="Group shots and uncertain figurine matches are queued here for one-click reassignment."
    ...
    >
      ...
      {rows.length === 0 ? (
        <section className="rounded-md border-[3px] border-[#7f1d1d]/70 bg-black/60 p-6 text-sm text-zinc-400 shadow-[inset_0_0_34px_rgba(0,0,0,0.65)]">
          No figurine sightings currently need review.
        </section>
      ) : (
